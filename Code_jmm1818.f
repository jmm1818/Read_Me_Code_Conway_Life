...
: test_functioning
    grid @ 0 do grid @ 0 do values_x_y I J 
...
: go_create_random
  ...
  save_data
  begin
  map_bmp                             { Store window handle                             }
  test_functioning
  bmp-address @ bmp-to-screen-stretch { Stretch .bmp to display window                  }
  1000 ms
  key?                              { Delay for viewing ease, reduce for higher speed }                                { Break test loop on key press                    }
  until
  ; 
